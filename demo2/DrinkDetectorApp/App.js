...
import { SafeAreaView, ScrollView, Text, NativeModules, Image, StyleSheet, View } from 'react-native';
...
const App = () => {
  ...
  const handleOnPress = () => {
    ImagePicker.openPicker({ cropping: true }).then(async image => {
      const detectedLabel = await NativeModules.Detector.detect(image.sourceURL);
